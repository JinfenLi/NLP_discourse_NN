...
class Evaluator(object):
    ...
    def pred_parser(self, output_dir='./examples', parse_type=None, bcvocab=None, draw=True):
        ...
        with open(os.path.join(output_dir, parse_type, "processed_data.p"), 'rb') as file:
            doclist = pickle.load(file)
        ids = pd.read_excel("../data/Reddit/reddit.xlsx")['row_id'].values
        relations = list(other.class2rel.keys())
        results = []
        i = 0
        with open("rows.txt",'w') as file:
            for id, lines in zip(ids,doclist):
                # ----------------------------------------
                # Read *.merge file
                doc = Doc()
                relation_d = {rel:0.0 for rel in relations}
                doc.read_from_fmerge(lines)
                if len(doc.edu_dict) >= 2:
                    file.write(str(id)+"\n")
                    # ----------------------------------------
                    # Parsing
                    # pred_rst = self.parser.sr_parse(doc, self.isFlat, bcvocab)
                    ...
                    #     total = sum(relation_d.values())
                    #     relation_d = {k: round(v /sum(relation_d.values()),2) for k, v in relation_d.items()}
                    # print(i)
                    # i += 1
                    # results.append((id, relation_d, int(total),pred_rst.tree.max_depth))
